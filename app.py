...
.card { background: var(--surface); border: 1px solid var(--line); border-radius: 16px; padding: 14px; backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); }
...
.item.warn { border-color: rgba(251, 113, 133, .5); background: rgba(251, 113, 133, .12); }
...
.compatibility-row { display: grid; grid-template-columns: 1fr auto; gap: 8px; margin-top: 10px; }
.badge { border-radius: 999px; font-size: 12px; padding: 6px 10px; border: 1px solid var(--line); background: rgba(255,255,255,.08); }
@supports not ((backdrop-filter: blur(8px)) or (-webkit-backdrop-filter: blur(8px))) {
  .card { background: rgba(17, 26, 53, 0.92); }
}
body.lite-mode .hero,
body.lite-mode .card,
body.lite-mode .metric {
...
function safeStorageSet(key, value) {
  try {
    ...
    // ignored for restricted browser modes
  }
}

const state = { routes: [], bookings: [], parcels: [], recentSearches: safeStorageGet('recent-searches', []) };
const el = {
  ...
  compat: document.getElementById('compatibility-state'),
  lite: document.getElementById('lite-mode-toggle'),
...
function saveRecent(entry) {
  ...
  safeStorageSet('recent-searches', state.recentSearches);
  renderRecents();
}
...
function compatibilityLabel() {
  const checks = [];
  checks.push(window.fetch ? 'fetch✓' : 'fetch→XHR');
  checks.push(('backdropFilter' in document.body.style || 'webkitBackdropFilter' in document.body.style) ? 'blur✓' : 'blur→fallback');
  checks.push(window.localStorage ? 'storage✓' : 'storage×');
  return checks.join(' · ');
}

el.compat.textContent = compatibilityLabel();
el.lite.addEventListener('click', () => {
  document.body.classList.toggle('lite-mode');
  setStatus(document.body.classList.contains('lite-mode') ? 'Включён облегчённый режим совместимости' : 'Облегчённый режим отключён');
});
  ...
  <section class='layout'>
    <div id='dashboard' class='card'>
      ...
      <div class='compatibility-row'>
        <div class='item' id='compatibility-state'>Проверка совместимости...</div>
        <button id='lite-mode-toggle' class='ghost' type='button'>Lite режим</button>
      </div>
    </div>
